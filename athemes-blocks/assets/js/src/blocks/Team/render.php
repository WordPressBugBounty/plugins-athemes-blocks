...
$teamMembersAmount = isset( $attributes['teamMembersAmount'] ) ? $attributes['teamMembersAmount'] : $atts_defaults['teamMembersAmount']['default'];
...
// Slides output.
$slider_items = array();
for ( $i = 0; $i < $teamMembersAmount; $i++ ) {

    // Image.
    $image = Settings::get_inner_setting( 'image' . ($i + 1), 'image', $attributes, $atts_defaults, '' );
    $image_id = $image['id'] ?? '';
    $has_image = $image_id ? true : false;
    $image_output = $has_image ? wp_get_attachment_image( $image_id, $imageSize, false, array( 'class' => 'at-block-team__item-image' ) ) : '';
    ...
    if ( $imagePosition === 'top' || $imagePosition === 'left' || $imagePosition === 'right' ) {
        $slide_item_content_output .= "
            <div class='at-block-team__item-inner'>
                ". ( $has_image ? $image_output : '' ) ."
                <div class='at-block-team__item-content'>
                    <div>
                        <div class='at-block-team__item-name'>
                            {$name}
                        </div>
                        <div class='at-block-team__item-company'>
                            {$company}
                        </div>
                    </div>
                </div>
...
$swiper_options = array(
    ...
    'navigation' => ($teamMembersAmount > 1 && $teamMembersAmount > $columnsDesktop) && ($carouselNavigation === 'arrows' || $carouselNavigation === 'both') && $displayCarouselNavigation ? array(
        'enabled' => true,
        'nextEl' => 'at-block-nav--next',
        'prevEl' => 'at-block-nav--prev',
     ) : false,
    'pagination' => ($teamMembersAmount > 1 && $teamMembersAmount > $columnsDesktop) && ($carouselNavigation === 'dots' || $carouselNavigation === 'both') && $displayCarouselNavigation ? array(
        'enabled' => true,
...
$swiper_markup_options = array(
    ...
    'swiper_class' => 'at-block-team__swiper',
    'swiper_slide_class' => 'at-block-team__item',
);
...
// Alignment.
$wrapper_classes[] = 'at-block-team--' . $alignment;

// Vertical Alignment.
$wrapper_classes[] = 'at-block-team--vertical-alignment-' . $verticalAlignment;

// Image Position.
$wrapper_classes[] = 'at-block-team--image-' . $imagePosition;

// Image Style.
$wrapper_classes[] = 'at-block-team--image-style-' . $imageStyle;
